...
/* Warn if creds not set */
if (supabaseUrl.includes('YOUR') || supabaseAnonKey.includes('YOUR')) {
  status.hidden = false;               // red “!” badge in footer
...
/* ------------------------------------------------------------------
   4.  AUTH FLOWS (email + password only)
------------------------------------------------------------------ */
...
q('#signup-form').onsubmit = async (e) => {
  ...
  /* create profile + starter blog via RPC you already added */
...
async function onSignIn(user) {
  ...
  supa.channel('blogs')
      .on('postgres_changes',
          { event: '*', schema: 'public', table: 'blogs', filter: `owner_id=eq.${user.id}` },
          loadBlogs)
      .subscribe();
}
