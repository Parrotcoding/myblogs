...
const authSec = q('#auth-section'),
      dashSec = q('#dash-section'),
      postsPan = q('#posts-panel');
const authMsg = q('#auth-msg'),
      blogsUl = q('#blogs-list'),
      postsUl = q('#posts-list');
...
/* — ensures only login form visible on first paint — */
qa('form[id$="-form"]').forEach(f => f.id === 'login-form' ? show(f) : hide(f));
...
async function loadBlogs() {
  ...
  blogsUl.innerHTML = error
    ...
    : data
        .map(
          b =>
            `<li><button class="btn link blog-btn" data-id="${b.id}">${b.title}</button></li>`
        )
        .join('');
  blogsUl
...
q('#new-post-form').onsubmit = async e => {
  ...
  const [title, content] = [
    q('#post-title').value.trim(),
    q('#post-content').value.trim()
  ];
  ...
  if (file) {
    ...
    imgUrl =
      supa.storage.from('post-images').getPublicUrl(path).data.publicUrl;
  }
  const { error } = await supa
    .from('posts')
    .insert({ blog_id: blogId, title, content, image_url: imgUrl });
